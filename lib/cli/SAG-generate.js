...
program.version((0, _version.version)()).option("-d, --default", "default outputMode").option("-a, --apiOnly", "only output api function list").option("-A, --all", "output include FetchFastModel").command('<ip> [op]', 'user define').action(function (ip, op) {
	inputPath = ip;
	outputPath = op;
